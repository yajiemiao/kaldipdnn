...
working_dir=exp_pdnn/bnf_tandem_fbank_sat
gmmdir=exp/tri3
...
for set in train; do
  if [ ! -d $working_dir/data_ivec/${set} ]; then
    steps_pdnn/sat/make_feat_with_ivec.sh --nj 24 --cmd "$train_cmd" --ivec-type speaker \
...
for set in train; do
  if [ ! -d $working_dir/data_bnf/${set} ]; then
    steps_pdnn/make_bnf_feat.sh --nj 24 --cmd "$train_cmd" \
      --norm-vars false --splice-opts "--left-context=0 --right-context=0" --add-deltas false \
      $working_dir/data_bnf/${set} $working_dir/data_ivec/${set} $working_dir $working_dir/_log $working_dir/_bnf || exit 1
