...
working_dir=exp_pdnn_110h/dnn_sgmm
gmmdir=exp/tri4a
sgmmdir=exp/sgmm2_5a
...
# Alignment with SGMM on the training and validation data
if [ ! -d ${sgmmdir}_ali_100k_nodup ]; then
  steps/align_sgmm2.sh --nj 24 --cmd "$train_cmd" --transform-dir ${gmmdir}_ali_100k_nodup \
    data/train_100k_nodup data/lang $sgmmdir ${sgmmdir}_ali_100k_nodup || exit 1
fi
if [ ! -d ${sgmmdir}_ali_dev ]; then
  steps/align_sgmm2.sh --nj 12 --cmd "$train_cmd" --transform-dir ${gmmdir}_ali_dev \
    data/train_dev data/lang $sgmmdir ${sgmmdir}_ali_dev || exit 1
fi
...
if [ ! -f $working_dir/train.pfile.done ]; then
  steps_pdnn/build_nnet_pfile.sh --cmd "$train_cmd" --do-concat false \
    ...
    $working_dir/data/train ${sgmmdir}_ali_100k_nodup $working_dir || exit 1
  touch $working_dir/train.pfile.done
fi
if [ ! -f $working_dir/valid.pfile.done ]; then
  steps_pdnn/build_nnet_pfile.sh --cmd "$train_cmd" --do-concat false \
    --norm-vars false --splice-opts "--left-context=5 --right-context=5" \
    $working_dir/data/valid ${sgmmdir}_ali_dev $working_dir || exit 1
...
if [ ! -f  $working_dir/decode.done ]; then
  ...
  graph_dir=$sgmmdir/graph_sw1_tg
  steps_pdnn/decode_dnn.sh --nj 24 --scoring-opts "--min-lmwt 7 --max-lmwt 18" --cmd "$decode_cmd" \
     $graph_dir $working_dir/data/eval2000 ${sgmmdir}_ali_100k_nodup $working_dir/decode_eval2000_sw1_tg || exit 1;
