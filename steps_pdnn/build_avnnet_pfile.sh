...
alidir=$2
dir=$3
feat1dim=$4	#The first modality feature dimension
feat2dim=$5	#The second modality feature dimension
feat1ind=$(($feat1dim - 1))
feat2ind=$(($feat2dim - 1))
...
if $shuffle_scp; then 
  feats="ark,s,cs:apply-cmvn --norm-vars=$norm_vars --utt2spk=ark:$sdata/JOB/utt2spk scp:$sdata/JOB/cmvn.scp \"scp:cat $sdata/JOB/feats.scp | utils/shuffle_list.pl --srand ${seed:-777} |\" ark:- | "
  audiofeats="$feats select-feats 0-$feat1ind ark:- ark:- |"
  videofeats="$feats select-feats $feat1dim-$feat2ind ark:- ark:- |"
  audiosplice="$audiofeats splice-feats $splice_opts ark:- ark:- |"
  videosplice="$videofeats splice-feats $splice_opts ark:- ark:- |"
  feats="paste-feats ark:$audiosplice ark:$videosplice ark:-"
fi
