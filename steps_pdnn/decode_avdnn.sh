...
if [ $# != 5 ]; then
   ...
   echo "Usage: steps/decode_dnn.sh [options] <graph-dir> <data-dir> <ali-dir> <decode-dir> <scr-dir>"
   echo " e.g.: steps/decode_dnn.sh exp/tri4/graph data/test exp/tri4_ali exp/tri4_dnn/decode KaldiScripts"
...
scoredir=$5			#directory of scoring script
...
midfeats="$feats nnet-forward --no-softmax=true $srcdir/dnn.nnet.bran ark:- ark:-|"
finalfeats="$midfeats nnet-forward --class-frame-counts=$dir/class.counts --apply-log=true --no-softmax=false $srcdir/dnn.nnet.main ark:- ark:- |"
...
if ! $skip_scoring ; then
  [ ! -x $scoredir/iskld_score_kaldi.sh ] && \
    echo "$0: not scoring because local/score.sh does not exist or not executable." && exit 1;
  $scoredir/iskld_score_kaldi.sh $scoring_opts --cmd "$cmd" $data $graphdir $dir
fi
