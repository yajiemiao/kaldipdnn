...
# Config for ivector
ivec_type=speaker     # the type of appended features: speaker, utterance, frame
...
$cmd JOB=1:1 $dir/log/get_feat_dim.log \
  feat-to-dim "ark,s,cs:copy-feats scp:$dir/append_feat.$name.1.scp ark:- | subset-feats --n=1 ark:- ark:- |" ark,t:$dir/feat_dim || exit 1;
...
if [ $stage -le 3 ]; then
  $cmd JOB=1:$nj $dir/log/build_pfile.JOB.log \
    build-pfile-from-ali --every-nth-frame=$every_nth_frame $alidir/final.mdl "ark:gunzip -c $alidir/ali.JOB.gz|" \
      scp:$dir/append_feat.$name.JOB.scp "|$PPATH/pfile_create -i - -o  $dir/$name.pfile.JOB -f $feat_dim -l 1" || exit 1;
