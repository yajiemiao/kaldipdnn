...
case $ivec_type in
  ...
  frame)  ivfeats="ark:copy-feats scp:$ivdir/ivector.scp ark:- |";;
  *) echo "$0: invalid ivector type $ivec_type" && exit 1;
esac
ivfeats="$ivfeats nnet-forward $srcdir/dnn.nnet.adapt ark:- ark:- |"
...
finalfeats="ark:nnet-forward --class-frame-counts=$dir/class.counts --apply-log=true --no-softmax=false $srcdir/dnn.nnet.si scp:$dir/add_feat.$name.JOB.scp ark:- |"
